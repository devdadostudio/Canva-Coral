...
if (is_admin()) {
	...
	if (!empty($action_link) && !empty($action_link['target']) && '_blank' === $action_link['target']) {
	...
	// button_mode: default "button" se il campo è vuoto/non impostato
	...
	if (!empty($action_link) && !empty($action_link['url']) && !empty($action_link['title'])) :
		...
		<div id="<?php echo esc_attr($id); ?>"
			...
			echo canva_get_action_link([
				...
				'target'                => $target_link, // contiene già target + rel
