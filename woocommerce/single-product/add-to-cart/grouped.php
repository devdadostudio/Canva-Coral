...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
	<table cellspacing="0" class="woocommerce-grouped-product-list group_table">
		<tbody>
			...
			foreach ( $grouped_products as $grouped_product_child ) {
				...
				echo '<tr id="product-' . esc_attr( $grouped_product_child->get_id() ) . '" class="woocommerce-grouped-product-list-item h-12 ' . esc_attr( implode( ' ', wc_get_product_class( '', $grouped_product_child ) ) ) . '">';
	...
	<?php if ( $quantites_required && $show_add_to_cart_button ) : ?>
		...
		<button type="submit" class="grouped_add_to_cart_button button alt mt-8"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
