...
<div class="_add-to-cart-buttons flex flex-wrap justify-between">
	...
	?>
		<form class="variations_form cart mb-6" action="<?php echo esc_url(apply_filters('woocommerce_add_to_cart_form_action', $product->get_permalink())); ?>" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint($product->get_id()); ?>" data-product_variations="<?php echo $variations_attr; ?>">
				...
				<?php } else { ?>
					...
					<div class="_ebook flex flex-col">
						...
						<button type="submit" class="single_add_to_cart_button ajax_add_to_cart button hollow w-48" type="submit" data-productid="<?php echo $variation_id; ?>" data-price="<?php echo esc_attr($variation_price); ?>">
							<div class="_add-to-cart-icon _icon-wrap">
								<figure class="_shopping-cart-icon absolute w-16">
									<?php echo canva_get_svg_icon('icons/ebook', 'w-4 mr-2 _shopping-cart-icon absolute'); ?>
								</figure>
